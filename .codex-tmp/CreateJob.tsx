...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  // Invoice fields are handled separately below â€” don't send them as job fields
  delete payload.generateInvoice
  delete payload.invoiceDueDate
  delete payload.invoiceLineItems
  // FormData is all strings â€” restore the real types
  ...
  // Advanced options â€” openToClaims/requiresApproval are always present
  ...
  try {
    ...
    if (raw.generateInvoice === 'true' && raw.invoiceLineItems) {
      try {
        ...
        // The job saved either way â€” surface the invoice failure separately
...
export function CreateJob() {
  ...
  // RHF's isSubmitting only covers client-side validation â€” it flips back to
  ...
  if (geofenceMode) advancedSummaryParts.push(`Clock-in location: ${geofenceMode} Â· ${geofenceRadius}m`)
  const advancedSummary = advancedSummaryParts.length > 0
    ? advancedSummaryParts.join(" Â· ")
    : "Using company defaults Â· No supervisor assigned"
  ...
  const [chargeRate_, setChargeRate] = useState<number | null>(null)
  ...
  const handleClientSelect = (client: ComboboxClient | null) => {
    ...
    if (client?.defaultChargeRate) {
      if (chargeRate && chargeRate !== client.defaultChargeRate) {
        // Capture the rate as it stood before offering to overwrite it â€”
        // this is what the prompt below shows as "current rate".
        setChargeRate(chargeRate)
        setShowApplyRate(true)
      } else {
        setChargeRate(client.defaultChargeRate)
      }
    }
  }
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              <Input
                label="Job Name"
                placeholder="e.g. Canary Wharf Security â€” Night Shift"
                {...register("title")}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-0">
              <ClientCombobox
                ...
                onToast={msg => { setClientToast(msg); setTimeout(() => setClientToast(null), 3000) }}
              />
              {/* RHF only tracks selectedClient in component state â€” this is
                  what actually reaches the <Form>'s FormData on submit. */}
              <Input type="hidden" name="client" value={selectedClient?._id ?? ''} />
              {showApplyRate && selectedClient && chargeRate_ !== null && (
                <div className="mt-2">
                  <ApplyRatePrompt
                    client={selectedClient}
                    currentRate={chargeRate_}
                    onKeep={() => setShowApplyRate(false)}
                    onApply={() => { setChargeRate(selectedClient.defaultChargeRate ?? 0); setShowApplyRate(false) }}
                  />
                </div>
              )}
              {clientToast && (
                <p className="mt-1.5 text-xs text-emerald-600 font-semibold">{clientToast}</p>
              )}
            {/* </div> */}
    ...
    {address && (
      <>
        ...
        <p className="mt-1 text-[11px] text-slate-400">
          Only the general area is shown to workers before they're assigned â€” the exact address is used for directions once someone is.
        </p>
      </>
    )}
    ...
    {shiftHours > 0 && (
      <div className="mt-3 bg-blue-50 border border-blue-100 rounded-lg px-4 py-2.5 flex items-center gap-2">
        ...
        <p className="text-sm text-blue-700">
          ...
          {endTime < startTime && <span className="text-blue-500"> Â· overnight</span>}
        </p>
      </div>
    )}
  </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-0">
      <div className="min-w-0">
        <Input
          ...
          min="0"
          placeholder="0.00"
          icon={<span className="text-slate-400 text-xs font-semibold">Â£</span>}
      ...
      <div className="min-w-0">
        ...
        <p className="text-[11px] text-slate-400 mt-1">
          ...
          {selectedWorkers.length < (requiredWorkers || 1) && " â€” the rest stay open"}
        </p>
    ...
    <label className="flex items-center gap-3 cursor-pointer">
      ...
      <span className="flex-1 min-w-0">
        ...
        <span className="block text-xs text-slate-400 mt-0.5">
          Creates a draft using the client and workers above â€” send it once the job's done
        </span>
      </span>
    </label>
  ...
  {/* Hidden Inputs â€” <Form> submits from the DOM, not RHF state */ }
  ...
  {/* Advanced â€” collapsed by default so the common case stays a short form */ }
      ...
      {advancedOpen && (
        ...
        >
          <div className="px-6 pb-6 pt-1 min-w-0 flex flex-col gap-6">
            ...
            {/* Internal notes â€” manager-only, never shown to workers. Worker-visible
            ...
            <div className="min-w-0 pt-6 border-t border-[#F1F5F9]">
              ...
              <p className="text-[11px] text-slate-400 mb-2">
                Internal only â€” never shown to workers.
              </p>
              <Textarea
                {...register("notes")}
              ...
              {[
                {
                  ...
                  sub: "Whatever's set in Settings â€” recommended",
                },
                {
                  value: "off",
                ...
                },
                {
                  ...
                  sub: "Blocks clock-in outside the radius â€” they'll need you to override it",
                },
              ].map(opt => (
                <label
                  key={opt.value}
            ...
            <AnimatePresence initial={false}>
              {geofenceMode && geofenceMode !== "off" && (
                ...
                >
                  ...
                  />
                  <p className="text-[11px] text-slate-400 mt-1">
                    Metres from the site. Phone GPS is often 50â€“100m out indoors, so anything
            ...
            {!coordinates && geofenceMode && geofenceMode !== "off" && (
              <p className="mt-3 text-xs text-amber-600">
                Pick a location above first â€” without map coordinates there's nothing to
  ...
  <div className="flex items-center gap-3 justify-end pt-2 pb-6">
    ...
    <Button type="submit" name="status" value="published" disabled={isSubmitting || isFormSubmitting}>
      {isFormSubmitting ? 'Publishingâ€¦' : 'Publish Job'}
    </Button>
  </div>
      </Form >
    </div >
